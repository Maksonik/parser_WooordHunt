...
from constants import WOOORDHUNT_URL, PART_OF_SPEECH, WOOORDHUNT_WORD_URL


def get_word(url_word):
    """Получить подробное описание слова"""
    correct_url = _remake_url_word_in_validated(url_word)
    page_word = _get_page(correct_url)
    if page_word:
        word = _parser_word(page_word)
        return word
